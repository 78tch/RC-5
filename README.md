...
メモリは01から99まである。  
前もって必要な設定をしてそれをメモリーに保存して、わかりやすい名前を付け、メモリ番号で呼び出して使う。
...
|2|バッキングトラックを再生する|BOSS TONE STUDIO|メモリ番号のフォルダに音声ファイルを置く|対応するファイル形式はWAV（44.1kHz、ステレオ32 ビット浮動小数点）、１トラック最大約1.5時間、メモリー合計最大約13時間|
|3|再生回数を１回か繰り返しか|LOOP|1SHOT|ONで１回再生、OFFで繰り返し再生|
|4|トラックの再生音量|LOOP|LEVEL|100から上下させる。再生中に［ MEMORY/LOOP LEVEL］つまみを押し込んで回す。|
...
|9|リズムをループ再生時に鳴らさない|RHYTHM|STOP|REC END にする。LOOP STOPで再生停止でリズムも停止。|
|10|メモリに名前を付ける|NAME|NAME|LOOP→RHYTHM→NAMEと切り替え、［MEMORY/LOOP LEVEL］つまみを回して文字を入力する位置にカーソルを合わせ、押し込み、回して文字を選び、決まったら押します。|
|11|メモリーを保存する|||［ SETUP］ボタンと［MEMORY］ボタンを同時に押し、WRITEを選んで押し込む。ほかのメモリーに保存することもできる。|
|12|メモリーを削除する|||［ SETUP］ボタンと［MEMORY］ボタンを同時に押し、WRITEをCLEARに切り替えて押し込む。|
