...
|9|メモリに名前を付ける|NAME|NAME|LOOP→RHYTHM→NAMEと切り替え、［MEMORY/LOOP LEVEL］つまみを回して文字を入力する位置にカーソルを合わせ、押し込み、回して文字を選び、決まったら押します。|
|10|メモリーを保存する|||［ SETUP］ボタンと［MEMORY］ボタンを同時に押し、WRITEを選んで押し込む。ほかのメモリーに保存することもできる。|
|11|メモリーを削除する|||［ SETUP］ボタンと［MEMORY］ボタンを同時に押し、WRITEをCLEARに切り替えて押し込む。|
